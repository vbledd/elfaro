            ...
            @foreach ($noticias as $listado)
                <div class="mainSlider" style="background-image: url('{{$url}}/img/{{ $listado->imagen}}');">
                    <div class="sliderContent">
                        <h1><a href="{{$url}}/noticia/{{$listado->id}}">{{$listado->titulo}}</a></h1>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    @foreach ($noticias as $listado)
        ...
        <div class="cardNotice">
            ...
            <div class="cardNoticeContent">
                <div class="cardNoticeContentCategoria">
                    @if($listado->estado == 1)
                    <span class="cardNoticeContentCategoriaNuevo">Nuevo</span>
                    @endif
                    <span>{{$listado->getCategoria->nombre}}</span>
                </div>
                <div class="cardNoticeContentTitulo"><a href="{{$url}}/noticia/{{$listado->id}}">{{$listado->titulo}}</a></div>
